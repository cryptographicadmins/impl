...
func setupA(t *testing.T) AdminStateTest {
	stateTest := AdminStateTest{}
	stateTest.keyPackages = make([]KeyPackage, totalGroupSize)
	scheme := suite.Scheme()

	for i := 0; i < totalGroupSize; i++ {
		// cred gen
...
func setupGroupA(t *testing.T) AdminStateTest {
	stateTest := setupA(t)
	var states []AdminState
	// start with the group creator
	s0, err := NewEmptyAdminState(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], stateTest.keyPackages[0])
	require.Nil(t, err)
	...
	for i := 1; i < totalGroupSize; i++ {
		s, err := NewJoinedAdminState(stateTest.initSecrets[i], stateTest.identityPrivs[i:i+1], stateTest.keyPackages[i:i+1], *welcome)
		require.Nil(t, err)
...
func setupGroupWithAdmins(t *testing.T) AdminStateTest {
	stateTest := setupA(t)
	var states []AdminState
	// start with the group creator
	s0, err := NewEmptyAdminState(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], stateTest.keyPackages[0])
	...
	// Verify that the states are all equivalent
	for _, lhs := range states {
		for _, rhs := range states {
			require.True(t, lhs.Equals(rhs))
		}
	}

	adminadds := make([]*MLSPlaintext, adminGroupSize)
	for i := 1; i < adminGroupSize; i++ {
...
func TestStateTwoPersonA(t *testing.T) {
	stateTest := setupA(t)
	// creator's state
	first0, err := NewEmptyAdminState(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], stateTest.keyPackages[0])
	require.Nil(t, err)
	...
	second1, err := NewJoinedAdminState(stateTest.initSecrets[1], stateTest.identityPrivs[1:2], stateTest.keyPackages[1:2], *welcome)
	require.Nil(t, err)
...
func TestStateExtensionsA(t *testing.T) {
	stateTest := setupA(t)
	...
	_, err := NewEmptyAdminStateWithExtensions(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], kpA, groupExtensions)
	require.Error(t, err)
	...
	alice0, err := NewEmptyAdminStateWithExtensions(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], kpA, groupExtensions)
	require.Nil(t, err)
...
func TestStateMarshalUnmarshalA(t *testing.T) {
	// Create Alice and have her add Bob to a group
	stateTest := setupA(t)
	alice0, err := NewEmptyAdminState(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], stateTest.keyPackages[0])
	require.Nil(t, err)
	...
	bob1, err := NewJoinedAdminState(stateTest.initSecrets[1], stateTest.identityPrivs[1:2], stateTest.keyPackages[1:2], *welcome1)
	require.Nil(t, err)
	...
	alice1aPriv := AdminStateSecrets{}
	_, err = syntax.Unmarshal(alice1priv, &alice1aPriv)
	require.Nil(t, err)

	alice1a, err := NewAdminStateFromWelcomeAndSecrets(*welcome1, alice1aPriv)
	require.Nil(t, err)
...
func TestStateMultiA(t *testing.T) {
	stateTest := setupA(t)
	// start with the group creator
	s0, err := NewEmptyAdminState(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], stateTest.keyPackages[0])
	require.Nil(t, err)
...
func TestStateUpdateA(t *testing.T) {
	stateTest := setupGroupA(t)
...
func TestStateRemoveA(t *testing.T) {
	stateTest := setupGroupA(t)
	for i := totalGroupSize - 2; i > 0; i-- {
...
func TestStateAdminTwoParties(t *testing.T) {
	// Create Alice and have her add Bob to a group
	stateTest := setupA(t)
	alice0, err := NewEmptyAdminState(groupID, stateTest.initSecrets[0], stateTest.identityPrivs[0], stateTest.keyPackages[0])
	require.Nil(t, err)
	...
	adminAdd, err := alice2.AdminAdd(bob2.Index)
	require.Nil(t, err)
	_, err = alice2.Handle(adminAdd)
	require.Nil(t, err)

	secret = randomBytes(32)
	commit2, _, alice3, err := alice2.Commit(secret)
//	_, _, _, err = alice2.Commit(secret)
	require.Nil(t, err)

	_, err = bob2.Handle(adminAdd)
	require.Nil(t, err)

	bob3, err := bob2.Handle(commit2)
	require.Nil(t, err)

	// Verify that the two states are equivalent
	require.True(t, alice3.Equals(*bob3))

	require.Equal(t, alice3.NumAdmins, uint32(2))

}

func TestStateUpdateAdmins(t *testing.T) {
	stateTest := setupGroupWithAdmins(t)
	states := stateTest.states
...
//		newKP, err := NewKeyPackageWithInitKey(suite, newInitKey.PublicKey, newCred, newPriv)
//		require.Nil(t, err)
		newKP, err := NewKeyPackageWithInitKey(suite, stateTest.keyPackages[i].InitKey, newCred, newPriv)
	...
	for _, lhs := range stateTest.states {
		...
		require.Equal(t, lhs.NumAdmins, uint32(adminGroupSize))
	}
}

func TestStateRemoveAdminStatus(t *testing.T) {
	stateTest := setupGroupWithAdmins(t)
	states := stateTest.states
	// Assumes there are at least 3 admins
	adminremove, err := states[1].AdminRemove(states[2].Index)
	require.Nil(t, err)
	_, err = states[1].Handle(adminremove)
	require.Nil(t, err)

	// commit the adds
	secret := randomBytes(32)
	com, _, next2, err := states[1].Commit(secret)
	require.Nil(t, err)
	states[1] = *next2
	...
	for i := 0; i < totalGroupSize; i++ {
		...
		_, err = states[i].Handle(adminremove)
		require.Nil(t, err)
		// Commit
		st, err := states[i].Handle(com)
		require.Nil(t, err)
	...
	for _, lhs := range stateTest.states {
		...
		require.Equal(t, lhs.NumAdmins, uint32(adminGroupSize - 1))
	}
}

// Here we make two proposals: one for removing admin
// status, and the other for removing the user
// TODO: test where we only remove the user
func TestStateRemoveAdmin(t *testing.T) {
	removeIndex := 2
	stateTest := setupGroupWithAdmins(t)
	states := stateTest.states
	// Assumes there are at least (removeIndex + 1) admins
	adminremove, err := states[1].AdminRemove(states[removeIndex].Index)
	require.Nil(t, err)
	_, err = states[1].Handle(adminremove)
	require.Nil(t, err)

	remove, err := states[1].Remove(states[removeIndex].Index)
	require.Nil(t, err)
	_, err = states[1].Handle(remove)
	require.Nil(t, err)
	...
	require.Nil(t, err)
	states[1] = *next2
	require.Equal(t, states[1].NumAdmins, uint32(adminGroupSize - 1))

	for i := 0; i < totalGroupSize; i++ {
		if i == 1 || i == removeIndex {
			continue
		}
		_, err = states[i].Handle(adminremove)
		require.Nil(t, err)
		_, err = states[i].Handle(remove)
		require.Nil(t, err)
		// Commit
		st, err := states[i].Handle(com)
		require.Nil(t, err)
		states[i] = *st
		require.True(t, states[1].Equals(states[i]))
		require.Equal(t, states[i].NumAdmins, uint32(adminGroupSize - 1))
	}
}
