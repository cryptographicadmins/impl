...
const (
	...
	ProposalTypeAdminAdd     ProposalType = 4
	ProposalTypeAdminUpdate  ProposalType = 5
	ProposalTypeAdminRemove  ProposalType = 6
)

func (pt ProposalType) ValidForTLS() error {
	return validateEnum(pt, ProposalTypeAdd, ProposalTypeUpdate, ProposalTypeRemove, ProposalTypeAdminAdd, ProposalTypeAdminUpdate, ProposalTypeAdminRemove)
}
...
type AdminAddProposal struct {
	Added LeafIndex
}

type AdminUpdateProposal struct {
	KeyPackage KeyPackage
	NewAdminKey SignaturePublicKey
}

type AdminRemoveProposal struct {
	Removed LeafIndex
}
...
func (p Proposal) Type() ProposalType {
	...
	case p.AdminAdd != nil:
		return ProposalTypeAdminAdd
	case p.AdminUpdate != nil:
		return ProposalTypeAdminUpdate
	case p.AdminRemove != nil:
		return ProposalTypeAdminRemove
	default:
...
func (p Proposal) MarshalTLS() ([]byte, error) {
	...
	case ProposalTypeAdminAdd:
		err = s.Write(p.AdminAdd)
	case ProposalTypeAdminUpdate:
		err = s.Write(p.AdminUpdate)
	case ProposalTypeAdminRemove:
		err = s.Write(p.AdminRemove)
	default:
...
func (p *Proposal) UnmarshalTLS(data []byte) (int, error) {
	...
	case ProposalTypeAdminAdd:
		p.AdminAdd = new(AdminAddProposal)
		_, err = s.Read(p.AdminAdd)
	case ProposalTypeAdminUpdate:
		p.AdminUpdate = new(AdminUpdateProposal)
		_, err = s.Read(p.AdminUpdate)
	case ProposalTypeAdminRemove:
		p.AdminRemove = new(AdminRemoveProposal)
		_, err = s.Read(p.AdminRemove)
	default:
...
type Commit struct {
	...
	AdminUpdates []ProposalID `tls:"head=2"`
	AdminRemoves []ProposalID `tls:"head=2"`
	AdminAdds    []ProposalID `tls:"head=2"`
...
func (pt MLSPlaintext) toBeSignedA(ctx AdminGroupContext) []byte {
	s := syntax.NewWriteStream()
	err := s.Write(ctx)
	if err != nil {
		panic(fmt.Errorf("mls.mlsplaintext: grpCtx marshal failure %v", err))
...
func (pt *MLSPlaintext) signA(ctx AdminGroupContext, priv SignaturePrivateKey, scheme SignatureScheme) error {
	tbs := pt.toBeSignedA(ctx)
	sig, err := scheme.Sign(&priv, tbs)
	if err != nil {
		return err
	}
...
func (pt *MLSPlaintext) verifyA(ctx AdminGroupContext, pub *SignaturePublicKey, scheme SignatureScheme) bool {
	tbs := pt.toBeSignedA(ctx)
	return scheme.Verify(pub, tbs, pt.Signature.Data)
}
...
func (gi AdminGroupInfo) dump() {
	...
	fmt.Printf("\tSignature %x\n", gi.Signature)
	fmt.Printf("\tAdmins %x\n", gi.Admins)
	fmt.Printf("\tNumAdmins %x\n", gi.NumAdmins)
	fmt.Printf("\n+++++ groupInfo +++++\n")
}

func (gi AdminGroupInfo) toBeSigned() ([]byte, error) {
	return syntax.Marshal(struct {
		GroupID                 []byte `tls:"head=1"`
		Epoch                   Epoch
		Tree                    TreeKEMPublicKey
...
func (gi *AdminGroupInfo) sign(index LeafIndex, priv *SignaturePrivateKey) error {
	// Verify that priv corresponds to tree[index]
	kp, ok := gi.Tree.KeyPackage(index)
	if !ok {
		return fmt.Errorf("mls.groupInfo: Attempt to sign from unoccupied leaf")
...
func (gi AdminGroupInfo) verify() error {
	// Get pub from tree[SignerIndex]
	kp, ok := gi.Tree.KeyPackage(gi.SignerIndex)
	if !ok {
		return fmt.Errorf("mls.groupInfo: Attempt to sign from unoccupied leaf")
...
func newAdminWelcome(cs CipherSuite, epochSecret []byte, groupInfo *AdminGroupInfo) *Welcome {
	// Encrypt the GroupInfo
	pt, err := syntax.Marshal(groupInfo)
	if err != nil {
		panic(fmt.Errorf("mls.welcome: GroupInfo marshal failure %v", err))
...
func (w Welcome) AdminDecrypt(suite CipherSuite, epochSecret []byte) (*AdminGroupInfo, error) {
	gikn := groupInfoKeyAndNonce(suite, epochSecret)
